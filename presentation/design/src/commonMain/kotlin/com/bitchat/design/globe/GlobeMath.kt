...
/** kotlin.math has no degree/radian helpers, and java.lang.Math is JVM-only. */
...
object GlobeMath {
    ...
    class PreparedProjector(centerLatDeg: Double, centerLonDeg: Double) {
        private val sinCenterLat = sin(toRadians(centerLatDeg)).toFloat()
        private val cosCenterLat = cos(toRadians(centerLatDeg)).toFloat()
        private val sinCenterLon = sin(toRadians(centerLonDeg)).toFloat()
        private val cosCenterLon = cos(toRadians(centerLonDeg)).toFloat()
    ...
    fun projectRaw(latDeg: Double, lonDeg: Double, centerLatDeg: Double, centerLonDeg: Double): Projection {
        ...
        val phi0 = toRadians(centerLatDeg)
        val dLambda = toRadians(normalizeLon(lonDeg - centerLonDeg))
    ...
    fun unproject(x: Double, y: Double, centerLatDeg: Double, centerLonDeg: Double): Pair<Double, Double>? {
        ...
        if (rho < 1e-9) {
            ...
            lat = toDegrees(asin(cosC * sin(phi0) + (-y) * sinC * cos(phi0) / rho))
            lonOffset = toDegrees(atan2(x * sinC, rho * cos(phi0) * cosC - (-y) * sin(phi0) * sinC))
        }
    ...
    fun autoPrecision(globeRadiusPx: Float, screenMinPx: Float): Int {
        ...
        for (p in 1..MAX_PRECISION) {
            val spanPx = (cellSpanLat(p) * (PI / 180.0) * globeRadiusPx).toFloat()
    ...
    fun zoomForPrecision(precision: Int, baseRadiusPx: Float, screenMinPx: Float): Float {
        val spanRad = cellSpanLat(precision) * (PI / 180.0)
