...
import com.bitchat.design.util.VoiceWaveformCache
...
fun WaveformPreview(
    ...
    LaunchedEffect(cached) {
        if (cached != null) {
            ...
            stateSamples.clear(); stateSamples.addAll(cached.toList())
        } else {
            // For now, generate a simple placeholder waveform
            val placeholder = FloatArray(120) { 0.3f + (it % 10) * 0.05f }
            stateSamples.clear(); stateSamples.addAll(placeholder.toList())
