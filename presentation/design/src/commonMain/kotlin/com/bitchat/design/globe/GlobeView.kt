...
fun GlobeView(
    ...
    val labelPainter = remember(textMeasurer) { GlobeLabelPainter(textMeasurer) }
    ...
    LaunchedEffect(state) {
        snapshotFlow { state.selectedGeohash }
            .drop(1)
            .collect {
                // Upstream used KEYBOARD_TAP with FLAG_IGNORE_GLOBAL_SETTING. Compose's
                // multiplatform haptics expose intent rather than a constant; TextHandleMove is
                // the closest short confirmation tick and is what the platform maps to a tap.
    ...
    Box(modifier = modifier) {
        ...
        Canvas(
            ...
            .pointerInput(state) {
                ...
                // wall clock with those semantics, so gesture times are milliseconds since a
                // monotonic origin taken when the gesture handler starts. Only differences matter.
                val gestureEpoch = TimeSource.Monotonic.markNow()
                fun nowMs(): Long = gestureEpoch.elapsedNow().inWholeMilliseconds
                var lastTapTime = 0L
        ...
        if (cityMaxRank == null || cityMaxRank >= 0) {
            drawCities(
                ...
                labelPainter, labelTextSizeSmall, density.density,
...
private fun DrawScope.drawCities(
    ...
    for (city in cities) {
        ...
        if (showLabels && (zoom >= 6f || (important && zoom >= 2.5f))) {
            ...
            with(painter) {
                drawLabel(
                    text = city.name, x = tx, y = sy, sizePx = textSize,
                    color = colors.label.copy(alpha = 0.78f * alpha),
                    haloColor = colors.labelHalo.copy(alpha = 0.55f * alpha),
...
private fun DrawScope.drawGeohashLabels(
    ...
    for (cell in cells) {
        ...
        val sx = cx + p.x * r
        val sy = cy + p.y * r
        val sizePx = if (isSelected) selectedSize else neighborSize
        val labelColor = if (isSelected) {
            colors.accent
        } else {
            colors.label.copy(alpha = 0.63f * p.cosC.coerceIn(0.4f, 1f))
        }
        val halo = colors.labelHalo.copy(alpha = if (isSelected) 0.78f else 0.47f)
        with(painter) {
            drawLabel(
                text = cell, x = sx, y = sy, sizePx = sizePx,
                color = labelColor, haloColor = halo,
                bold = isSelected, centered = true
            )
        }
    }
}
