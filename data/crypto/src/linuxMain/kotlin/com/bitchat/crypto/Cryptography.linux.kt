...
import libsodium.crypto_sign_ed25519_SECRETKEYBYTES
...
/**
 * Linux ARM64 cryptography implementation using libsodium and secp256k1 via cinterop.
 *
 * This implementation mirrors the Apple implementation but uses libsodium for
 * hashing and HMAC operations instead of CoreCrypto.
 */
...
actual object Cryptography {
    ...
    actual fun generateEd25519KeyPair(): Pair<String, String> {
        ...
        val publicKey = ByteArray(crypto_sign_ed25519_PUBLICKEYBYTES.toInt())
        val secretKey = ByteArray(crypto_sign_ed25519_SECRETKEYBYTES.toInt())
        memScoped {
    ...
    actual fun ed25519Sign(message: ByteArray, privateKeyHex: String): ByteArray {
        ...
        val signature = ByteArray(crypto_sign_ed25519_BYTES.toInt())
        memScoped {
        ...
        data.usePinned { dataPinned ->
            hash.usePinned { hashPinned ->
                crypto_hash_sha256(
                    ...
                    dataPinned.addressOf(0).reinterpret<uint8_tVar>(),
                    data.size.toULong()
                )
            }
        }
    ...
    actual fun encryptAESGCM(plaintext: String, key: ByteArray): ByteArray {
        ...
        // Check if AES-GCM is available (requires hardware support)
        if (crypto_aead_aes256gcm_is_available() != 1) {
            // Fall back to XChaCha20-Poly1305 which is always available
    ...
    actual fun decryptAESGCM(encryptedData: ByteArray, key: ByteArray): String? {
        return try {
            ...
            if (crypto_aead_aes256gcm_is_available() != 1) {
                // Fall back to XChaCha20-Poly1305
    ...
    private fun decryptAeadXChaCha(cipher: ByteArray, nonce: ByteArray, key: ByteArray): ByteArray {
        ...
        memScoped {
            ...
            check(result == 0) { "XChaCha20-Poly1305 decryption failed" }
    ...
    // Fallback encryption using XChaCha20-Poly1305 when AES-GCM hardware not available
    private fun encryptXChaCha(plaintext: String, key: ByteArray): ByteArray {
        val nonce = ByteArray(crypto_aead_xchacha20poly1305_ietf_NPUBBYTES.toInt())
        nonce.usePinned { pinned ->
            randombytes_buf(pinned.addressOf(0).reinterpret<uint8_tVar>(), nonce.size.toULong())
        }
        ...
        memScoped {
            ...
            check(result == 0) { "XChaCha20-Poly1305 encryption failed" }
            return nonce + cipher.copyOf(clen.value.toInt())
        }
    }

    // Fallback decryption using XChaCha20-Poly1305 when AES-GCM hardware not available
    private fun decryptXChaCha(encryptedData: ByteArray, key: ByteArray): String? {
        val nonceSize = crypto_aead_xchacha20poly1305_ietf_NPUBBYTES.toInt()
        if (encryptedData.size < nonceSize) return null
